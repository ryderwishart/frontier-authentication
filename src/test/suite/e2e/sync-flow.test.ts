...
suite('E2E: Complete Sync Flow Tests', () => {
    ...
    test('E2E: Full sync with heartbeat monitoring', async function() {
        ...
        const phases = [
            { phase: 'committing', duration: 1000, progress: { current: 3, total: 3, description: 'Committing 3 files' } },
            { phase: 'fetching', duration: 2000, progress: { current: 0, total: 0, description: 'Checking for remote changes' } },
            { phase: 'merging', duration: 1000, progress: { current: 1, total: 1, description: 'Merging remote changes' } },
            { phase: 'pushing', duration: 1500, progress: { current: 0, total: 0, description: 'Uploading changes' } }
        ];

        for (const phaseData of phases) {
            const startTime = Date.now();
            progressEvents.push({ phase: phaseData.phase, description: phaseData.progress.description });
            ...
            assert.strictEqual(status.status, 'active', `Phase ${phaseData.phase} should have active lock`);
        ...
        assert.ok(heartbeatUpdates >= 8, `Should have multiple heartbeat updates (got ${heartbeatUpdates})`);
    ...
    test('E2E: Concurrent sync attempt while sync in progress', async function() {
        ...
        try {
            ...
            assert.ok(lockStatus.age < 2000, 'Lock should be recent');
    ...
    test('E2E: CPU-intensive phase grace period', async function() {
        ...
        const status = await stateManager.checkFilesystemLock(testWorkspacePath);
        assert.strictEqual(status.status, 'active', 'Merging phase should be active within grace period');
    ...
    test('E2E: Slow network sync completes successfully', async function() {
        ...
        for (const progress of fetchProgress) {
            ...
            const status = await stateManager.checkFilesystemLock(testWorkspacePath);
            assert.strictEqual(status.status, 'active', `Should be active at ${progress.current}/${progress.total}`);
    ...
    test('E2E: Progress tracking through multiple phases with realistic timing', async function() {
        ...
        const syncPhases = [
            ...
            },
            {
                phase: 'fetching',
                steps: [
                    { current: 0, total: 0, description: 'Checking for remote changes', delay: 1000 },
                    { current: 42, total: 127, description: 'Receiving objects: 42/127', delay: 500 },
                    { current: 85, total: 127, description: 'Receiving objects: 85/127', delay: 500 },
                    { current: 127, total: 127, description: 'Receiving objects: 127/127', delay: 500 },
        ...
        for (const phaseData of syncPhases) {
            for (const step of phaseData.steps) {
                ...
                const status = await stateManager.checkFilesystemLock(testWorkspacePath);
                assert.strictEqual(status.status, 'active', `Should be active during ${step.description}`);
                assert.strictEqual(status.phase, phaseData.phase, `Phase should be ${phaseData.phase}`);
                assert.ok(status.progress, 'Progress should be present');
                assert.strictEqual(status.progress!.description, step.description, 'Description should match');
